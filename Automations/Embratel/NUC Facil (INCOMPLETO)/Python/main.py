...
warnings.filterwarnings("ignore", message="alguma mensagem de aviso específica")
...
class Main_Screen:
    def __init__(self, root):
        ...
        self.year_label = ttk.Label(self.convert_pack, text="Ano de referência:", )
        ...
        self.month_entry = DateEntry(self.convert_pack, date_pattern="dd/mm/yyyy")
        self.month_entry.pack(padx=10, pady=5)
        self.update_button = ttk.Button(self.convert_pack, text="Criar planilha", command=self.merge_files)
        ...
        self.open_convert_path_button = ttk.Button(self.options_pack, text='Abrir pasta de arquivos a converter', command=self.open_convert_files_path)
        self.open_convert_path_button.pack(padx=10, pady=5)
        self.open_destin_path_button = ttk.Button(self.options_pack, text='Abrir pasta de destino dos arquivos', command=self.open_destin_files_path)
        self.open_destin_path_button.pack(padx=10, pady=5)
        self.open_nuc_software_button = ttk.Button(self.options_pack, text='Abrir o software do NUC Fácil', command=self.open_nuc_software)
        self.open_nuc_software_button.pack(padx=10, pady=5)

    def show_custom_message(self, message:str, title:str = "Aviso", timeout:int = 5):
        ...
        custom_message_window.after(timeout*1000, custom_message_window.destroy)  # Fecha após 10 segundos
    ...
    def merge_files(self):
        try:
            ...
            self.show_custom_message(title='Erro', message=error_msg, timeout=25)
...
and cast(datahora as date) like '2023-09%'"""
    ...
    def get_data_datalens(self):        
        ...
        if df_datalens.empty:
            raise DataframeIsEmpty(f'O banco de dados do período de {self.start_date} a {self.end_date} está vazio')
    ...
    def move_snu_file(self):
        self.snu_files = [file for file in os.listdir(self.nuc_path) if "snu" in file.lower()]

        for file in self.snu_files:
            if '.CSV' in file:
                os.replace(join(self.nuc_path, file), join(self.convert_path, file))
    ...
    def treat_csv_snu_files(self, df_datalens: pd.DataFrame):
        ...
        for file in os.listdir(self.convert_path):
            ...
            if '.csv' in file.lower() or '.txt' in file.lower():
                temp_df = pd.read_csv(join(self.convert_path, file), sep=";", engine='pyarrow')
            ...
            temp_df = temp_df.loc[temp_df['TIPO'] == "Q"] #Filtra o tipo do chamado
            
            temp_df['DATA_CHAMADA'] = pd.to_datetime(temp_df['DATA_CHAMADA'], format='%d/%m/%Y').dt.date.astype(str)
            temp_df['DURACAO'] = temp_df['DURACAO'].astype(str).str.replace(',','.').astype(float)
            temp_df['VALOR'] = temp_df['VALOR'].astype(str).str.replace(',','.').astype(float)
            
            temp_df = temp_df[['DATA_CHAMADA', 'HORA', 'TEL_CHAMADO', 'TEL_ORIGEM', 'TIPO', 'DURACAO', 'VALOR']] #Organiza a ordem das colunas

            temp_df = temp_df.rename(columns={
                'DATA_CHAMADA': 'data',
                'HORA': 'hora',
                'TEL_CHAMADO': 'origem', # o TEL_CHAMADO e o TEL_ORIGEM são invertidos devido a um erro na planilha
                'TEL_ORIGEM': 'destino',
                'TIPO': 'categoria',
                'DURACAO': 'durtar',
    ...
    def format_worksheet_styles(self, ws, final_df):
        ...
        # Suponhamos que você deseja formatar a área de A1 até C10 como uma tabela
        ...
        tab = openpyxl.worksheet.table.Table(displayName="Tabela_0800", ref=f"{ws.cell(1, 1).coordinate}:{ws.cell(end_row, end_col).coordinate}")
    ...
    def create_worksheet(self, df):  # sourcery skip: raise-from-previous-error
        ...
        for r in dataframe_to_rows(df, index=False, header=True): #Insere DF no excel
            ws.append(r)
            
        ws = self.format_worksheet_styles(ws, df)

        # Salve o arquivo Excel
        path_file = join(self.destin_path, f'Relatório Electrolux - Ligações {dt.strptime(self.end_date, "%d/%m/%Y").strftime("%Y-%m")}.xlsx')
        try:
            wb.save(path_file)
        except PermissionError:
            raise ExcelIsOpened(f'Verifique se o arquivo  "Relatório Electrolux - Ligações {dt.strptime(self.end_date, "%d/%m/%Y").strftime("%Y-%m")}.xlsx" encontra-se aberto')
        
        self.show_custom_message(title="Sucesso", message=f"O relatório foi criado em {path_file}")
